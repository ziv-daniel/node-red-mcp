...
export class AuthorizationError extends AppError {
    constructor(action, resource = 'resource') {
...
export class NodeRedError extends AppError {
    nodeRedStatusCode;
    nodeRedResponse;
    constructor(message, statusCode = 500, nodeRedStatusCode, nodeRedResponse) {
 ...
 */
export function handleNodeRedError(error, context) {
    if (error.response) {
        ...
        if (error.message?.includes('Unexpected token') && contentType.includes('text/html')) {
            throw new NodeRedError(`Node-RED returned HTML instead of JSON in ${context}. This usually indicates an authentication redirect or configuration issue. Check your Node-RED admin settings and authentication.`, 502, status, { originalError: error.message, contentType, responsePreview: typeof data === 'string' ? data.substring(0, 200) : data });
        }
        throw new NodeRedError(`Node-RED API error in ${context}: ${data?.message || data?.error || 'Unknown error'}`, status >= 500 ? 502 : status, // Bad Gateway for server errors
        status, data);
    }
    else if (error.request) {
        // Network error
        throw new NodeRedError(`Failed to connect to Node-RED in ${context}: ${error.message}`, 503 // Service Unavailable
        );
    }
    else if (error.message?.includes('Node-RED returned HTML')) {
        // Our custom validation error
        throw new NodeRedError(`${error.message} (Context: ${context})`, 502 // Bad Gateway
        );
    }
    else {
        // Other error
 ...
 */
export function validateTypes(data, fieldTypes) {
    for (const [field, expectedType] of Object.entries(fieldTypes)) {
        if (data[field] !== undefined) {
            const actualType = typeof data[field];
            if (actualType !== expectedType) {
                throw new ValidationError(`Field '${field}' must be of type ${expectedType}, got ${actualType}`, field, data[field]);
            }
        }
    }
}
